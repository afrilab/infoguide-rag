...
from ingestion import extract_text, sanitize_document_stem, IMAGES_DIR
...
async def generate(req: GenerationRequest):
    ...
    answer, trust_score = generate_answer_with_gpt(req.query, internal_chunks, state.gpt())
...
@app.delete("/chat/document/{document_id}")
async def chat_delete_document(document_id: str):
    """Removes a globally-uploaded document and everything derived from it:
    its chunks, its slice of the embedding index, its raw file, and its
    extracted images — so deleting it in one chat cleans it up everywhere."""
    if document_id not in chat_documents:
        raise HTTPException(status_code=404, detail="Document not found")

    image_dirs_to_remove = set()
    ...
    if CHUNKS_PATH.exists():
        ...
        for c in all_chunks:
            ...
            if metadata.get("document_id") == document_id:
                ...
                if image_path:
                    image_dirs_to_remove.add(Path(image_path).parent)
                continue
            kept_chunks.append(c)
        with open(CHUNKS_PATH, "w", encoding="utf-8") as f:
            json.dump(kept_chunks, f, ensure_ascii=False, indent=4)
    ...
    if EMBEDDINGS_PATH.exists() and METADATA_PATH.exists():
        ...
        kept_metadata = [m for m, keep in zip(existing_metadata, keep_mask) if keep]
        kept_vectors = existing_vectors[keep_mask] if len(keep_mask) else existing_vectors

        np.save(EMBEDDINGS_PATH, kept_vectors)
        with open(METADATA_PATH, "w", encoding="utf-8") as f:
            json.dump(kept_metadata, f, ensure_ascii=False, indent=2)

    file_path = chat_documents[document_id].get("file_path")
    if file_path:
        raw_file = Path(file_path)
        if raw_file.exists():
            raw_file.unlink()
        image_dirs_to_remove.add(IMAGES_DIR / sanitize_document_stem(raw_file.stem))

    for image_dir in image_dirs_to_remove:
        if image_dir.exists() and image_dir.is_dir():
            shutil.rmtree(image_dir)

    del chat_documents[document_id]
...
async def chat_query(req: ChatQueryRequest):
    ...
    answer, trust_score = generate_answer_with_gpt(req.query, reranked, state.gpt())
