...
def show_generation_page():
        ...
        if st.button("Generate Final Answer"):
            try:
                with st.spinner("Generating answer with GPT-4o mini..."):
                    answer, trust_score = generate_answer_with_gpt(
                        query=st.session_state["original_query"],
                ...
                st.session_state["generated_trust_score"] = trust_score
    ...
    if st.session_state.get("generation_done"):
        ...
        trust_score = st.session_state.get("generated_trust_score")
        if trust_score is not None:
            st.caption(f"Trust score: {trust_score:.2f}")
