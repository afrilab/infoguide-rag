...
Project is HPC-ready. Setup once, then submit:
```bash
bash hpc/setup.sh
sbatch hpc/run_cpu.sh   # CPU pipeline
sbatch hpc/run_gpu.sh   # GPU finetuning
sbatch hpc/run_gpu_hybrid.sh   # GPU hybrid search
```
Logs: `logs/`, outputs: `outputs/`. See `hpc/README.md`.
